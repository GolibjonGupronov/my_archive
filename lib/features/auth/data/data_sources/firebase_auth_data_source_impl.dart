...
import 'package:my_archive/core/services/location_service.dart';
...
class FirebaseAuthDataSourceImpl extends AuthDataSource {
  ...
  Future<bool> registration(RegistrationParams params) async {
    ...
    final userCredential = await AliceFirebase.logCall(
      name: "auth/signInWithCredential",
    ...
    if (!hasPasswordProvider) {
      await AliceFirebase.logCall(
        name: "auth/linkWithCredential",
        request: {"email": email},
        action: () => user.linkWithCredential(EmailAuthProvider.credential(email: email, password: params.smsCode)),
      );
    }
    ...
    await AliceFirebase.logCall(
        name: FirebaseUrls.users,
        request: {"uid": uid, ...params.toMap},
        action: () => firestore.collection(FirebaseUrls.users).doc(uid).set(params.toMap));
  ...
  Future<String> sendLogin(LoginParams params) async {
    return await AliceFirebase.logCall(
      ...
      action: () async {
        final response =
            await firebaseAuth.signInWithEmailAndPassword(email: "${params.phone}@gmail.com", password: params.password);
        ...
        try {
          ...
          await firebaseAuth.verifyPhoneNumber(
            ...
            verificationCompleted: (PhoneAuthCredential credential) async {
              // Avtomatik tasdiqlansa, shu yerda ham completer.complete(true) qilish mumkin
            },
  ...
  Future<void> _saveDeviseSession(String uid) async {
    ...
    await AliceFirebase.logCall(
      ...
      action: () async {
        final sessionRef =
            firestore.collection(FirebaseUrls.users).doc(uid).collection(FirebaseUrls.deviceSessions).doc(deviceId);
